...
import type { CartListProps } from '../cart.type'
      ...
      <ul className={scss.cart_content_list}>
        {
          ...
          products.map(product => (
            <li key={product._id} className={scss.cart_content_item}>
              <Image className={scss.cart_product_icon} width={100} height={100} src={product.images[0]} alt={product.title}/>
              <aside className={scss.cart_product_data}>
                <button onClick={() => dispatch(removeFullProduct(product._id))} className={scss.cart_delete_product}>
                  <svg className={scss.cart_delete_product} viewBox="0 0 18 20" fill="none">
                ...
                <section>
                  ...
                  <p>{tr('product.rating')}: {product.rating}/5</p>
                  <p>{tr('product.in-stock')}: {product.stock}</p>
                  <p>{tr('product.in-cart')}: {product.count}</p>
                </section>
                {!isMobile && 
              ...
              {isMobile &&  <ProductCount product={product}/>}
            </li>
          )) : 
          <li className={scss.cart_empty_text}>{tr('cart.empty')}</li>
        }
      </ul>
    </div>
  )
