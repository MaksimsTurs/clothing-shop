...
export default function CartList({ products }: CartListProps) {
  ...
  return( 
    <div className={scss.cart_container}>
      <h4 className={scss.cart_title}>{tr('title')}</h4>
      <ul className={scss.cart_content_list}>
        {products && products.length > 0 ? 
          products.map(product => (
            <li key={product._id} className={scss.cart_content_item}>
              <div className={scss.cart_item_data}>
                <ExtendedIMG className={scss.cart_product_icon} width={100} height={100} src={product.images[0]} alt={product.title}/>
                <aside className={scss.cart_product_data}>
                  <button onClick={() => dispatch(removeProduct(product._id))} className={scss.cart_delete_product}>
                  ...
                  <section>
                    ...
                    <p>{tr('rating')}: {product.rating}/5</p>
                    <p>{tr('stock')}: {product.stock}</p>
                    <p>{tr('in-cart')}: {product.count}</p>
                  </section>
                </aside>
              </div>
              <div className={scss.cart_item_cost}>
                <ProductCost className={scss.cart_delete_margin} precent={product.precent} price={product.price}/>
                <ProductCount product={product}/>
              </div>
            </li>)) : 
          <li className={scss.cart_empty_text}>{tr('empty')}</li>}
      </ul>
    </div>
  )
}