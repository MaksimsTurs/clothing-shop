...
export type RemoveFrom = 'order' | 'product' | 'section'

export type FormProps<T> = { id?: string | null, isEdit?: boolean, defaultValues?: DefaultInputValues<T> }
export type DefaultInputValues<T> = Partial<Record<keyof T, any>>
...
export type DataItemsProps = { _key: string, data?: DataItem[] }
type DataItem = { count?: number } & ProductData

export type ProductSelectProps = { setProductIDs: Dispatch<SetStateAction<string[]>>, productsID: string[] }

export type SectionFormProps<T> = { currIDs?: string[] } & FormProps<T>