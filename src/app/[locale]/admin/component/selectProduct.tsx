    ...
    <ul className={scss.product_list_container}>
      {
        ...
        filteredArray.map(product => (
          <li 
            ...
            key={product._id}>
            <ExtendedIMG src={product.images[0]} alt={product.title} width={14400} height={14400} quality={100}/>
              <div>
                <h5 className={scss.product_item_name}>{product.title}</h5>
                <div className={scss.product_item_cost_data}>
                  <p className={scss.product_item_data}>Цена: {product.price} {CURR_CURRENCY}</p>
                  {precent ? <p className={scss.product_item_cost_with_precent}>(with {(precent * 100).toFixed(2)} % {(product.price - (product.price * precent)).toFixed(2)} {CURR_CURRENCY})</p> : null}
                </div>
                <p className={scss.product_item_data}>В наличии: {product.stock}</p>
              </div>
          </li>
        )) : <p className={scss.product_list_emtpy}>Нет продуктов:</p>
      }
    </ul>
  )
}