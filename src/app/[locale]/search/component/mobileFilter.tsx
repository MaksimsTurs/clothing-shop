import scss from '../scss/mobileFilter.module.scss'

import { useScopedI18n } from '@/i18n/client'

import Filter from './filter'

import type { MobileFilterProps } from '../search.type'

export default function MobileFilter({ filterState, setFilterState, setFilterVisible, isFilterVisible, refetch }: MobileFilterProps) {
	const tr = useScopedI18n('Search')

  const hiddeFilterContainer = (): void => setFilterVisible!(false)

  return(
  <div className={isFilterVisible ? scss.filter_mobile_container : `${scss.filter_mobile_container} ${scss.filter_mobile_container_hidden}`}>
    <div className={scss.search_filter_smaller_body}>
      ...
      <Filter isChild refetch={refetch} filterState={filterState} setFilterState={setFilterState} setFilterVisible={setFilterVisible}/>
    </div>
  </div>
  )
}