...
import scss from '../scss/filterContainerWrapper.module.scss'

import { type PropsWithChildren, useState } from 'react'
import type { FilterContainerWrapperProps } from '../search.type'

export default function FilterContainerWrapper({ title, children }: PropsWithChildren<FilterContainerWrapperProps>) {
  ...
  return(
    <section className={scss.filter_container_wrapper} onClick={showCurrFilter}>
      ...
      <div style={{ height: isVisible ? 'auto' : 0 }} className={scss.filter_body}>
        {children}
      </div>
    </section>
  )
